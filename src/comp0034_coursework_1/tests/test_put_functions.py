...
def test_trainer_put_function_1(client,trainer_json_a):
    ...
    print(response1.json)
    assert response1.status_code == 201
    obj1 = db.session.execute(db.select(Trainer).filter_by(Trainer_ID='a',password='********')).scalar()
    assert obj1 != None
    response2 = client.put(
        '/put/trainer',
        json= {
    ...
    assert response2.status_code == 201
    obj2 = db.session.execute( db.select(Player).filter_by(Trainer_ID='a', password='************')).scalar()
    assert obj2 != None
